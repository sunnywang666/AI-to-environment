/* ============================================================
   云的身体 · 开场仿 AI 聊天框
   输入问题（或点预设、或直接下滑）→ AI 打字回答这次回答的能耗估算。
   估算用查表 + 长度系数（标注"估算值"），不接真实模型。
   ============================================================ */
...
function estimate(text) {
  ...
  const wh = base.wh * f;
  const ml = base.ml * f;
  return { type: classify(text), wh, ml, micro: wh / MICRO_WH, bottles: ml / 519 };
}
...
function reply(text) {
  ...
  if (e.type === "video") {
    return `你要的这段视频，代价要大得多：大约 ${wh} 瓦时的电，相当于让微波炉转 ${t}。复杂任务，可以是一次简单问答的成百上千倍。（估算值）\n\n往下滑，看看这些"微不足道"加起来是什么。`;
  }
  if (e.type === "image") {
    return `生成这张图，我大约要消耗 ${wh} 瓦时——相当于微波炉转 ${t}，再蒸发掉约 ${fmtMl(e.ml)} 毫升的冷却水。（估算值）\n\n而这，只是开始。往下滑。`;
  }
  const water =
    e.ml >= 100
      ? `，再用掉约 ${fmtMl(e.ml)} 毫升水来冷却——差不多 ${e.bottles < 1.2 ? "一" : e.bottles.toFixed(1)} 瓶矿泉水`
      : "，顺带蒸发掉一小口冷却水";
  return `回答你这句话，我大约要消耗 ${wh} 瓦时的电——相当于让一台微波炉转 ${t}${water}。（估算值）\n\n听起来微不足道？可把它乘以全世界每天几十亿次提问……往下滑，我带你看。`;
}
  ...
  function typeOut(str) {
    ...
    (function step() {
      if (i < str.length) {
        ...
        setTimeout(() => cursor && cursor.remove(), 1200);
      }
    })();
  }
